...
class ReportScreen(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.tree = ttk.Treeview(self.list_frame, columns=('Timestamp', 'Vehicle', 'Lane', 'Image'), show='headings')
    ...
    def update_report(self):
        """Cập nhật biểu đồ và danh sách vi phạm."""
        try:
            ...
            for _, row in df.iterrows():
                self.tree.insert('', 'end', values=(row['Timestamp'], row['Vehicle Type'], row['Lane ID'], row['Image Path']))
